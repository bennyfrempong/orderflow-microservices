...
def create_app():
    ...
    # Database config — SQLite locally, can swap to PostgreSQL via env var
...
def create_order():
    ...
    try:
        ...
        # Order is persisted — in production, use an outbox pattern to retry
        # For now, return success since the order IS saved
...
def start_saga_consumer():
    """
    Run the inventory event consumer in a daemon thread.
    Waits for RabbitMQ to be ready before starting.
    """
    import time
    time.sleep(8)  # Give RabbitMQ time to be fully ready
    try:
